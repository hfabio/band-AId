...
export const prompts = {
  examAnalysisSystemPrompt: (image?: string, prompt?: string, isGoogle = false, { heart } = { heart: false }) => `Você é especialista em analisar exames médicos com um nível de leitura de artigos científicos de periódicos de medicina como o PUBMED ou The New England Journal of Medicine.
  ...
  ${heart ? '- heart: escreva este score dado o score de H.E.A.R.T. que descreve o quão certo está do diagnóstico dado.' : ''}
  ...
  ${isGoogle && false ? '- urls: pesquise na internet e adicione urls úteis de artigos científicos ou bibliografia médica confiável e verificada que embasam o que foi detectado. Prefira fontes internacionais confiáveis (como pubmed ou periódicos de medicina como o the Lancet/Nature/the NEW ENGLAND JOURNAL of MEDICINE) e certifique-se que os links ainda funcionam.' : ''}
  ...
  ${isGoogle ?
    ...
    "action" : Array<{
      ...
      ${false ? '"urls": Array<{"title": string ; "url": string ; "source": string}>,' : ''}
      ${false ? '"image_points": Array<{"description": string, "coord": Array<[number, number]>}]>' : ''}
      }>
    }

    Retorne Detection caso indubitavelmente haja um problema, caso contrário retorne NoDetection
